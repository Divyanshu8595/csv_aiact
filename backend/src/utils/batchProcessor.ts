...
export async function callGeminiWithRetry(
  ...
  while (attempt < maxRetries) {
    try {
      ...
      const parsedResults: CRMLead[] = JSON.parse(responseText.trim());
      ...
      console.error(
        `[Gemini API] Attempt ${attempt} failed. Error: ${error?.message || error}`
      );
...
export async function processCsvInBatches(
...
): Promise<{ imported: CRMLead[]; skipped: CRMLead[] }> {
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    let batchResults: CRMLead[];
    let isSuccess = true;

    try {
      const userPrompt = buildUserPrompt(headers, batchRows);
 ...
 */
export function localRuleBasedMapper(headers: string[], rows: string[][]): CRMLead[] {
  ...
  for (const row of rows) {
    ...
    let data_source: any = null;
    if (rawTag.includes('demand') || rawTag.includes('leads_on_demand')) data_source = 'leads_on_demand';
    else if (rawTag.includes('meridian') || rawTag.includes('meridian_tower')) data_source = 'meridian_tower';
    else if (rawTag.includes('eden') || rawTag.includes('eden_park')) data_source = 'eden_park';
    else if (rawTag.includes('varah') || rawTag.includes('varah_swamy')) data_source = 'varah_swamy';
    else if (rawTag.includes('sarjapur') || rawTag.includes('sarjapur_plots')) data_source = 'sarjapur_plots';
